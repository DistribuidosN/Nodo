...
\begin{abstract}
Este documento describe el diseño y la implementación de un subsistema de nodos \emph{worker} para procesamiento distribuido de imágenes en paralelo. El prototipo fue desarrollado en Python con \texttt{asyncio} y \texttt{grpc.aio}, y contempla tres nodos worker, un clúster de coordinadores activo/pasivo con failover real, cola central durable, ownership global por solicitud, cola de prioridad por nodo, scheduling interno, control de recursos, backpressure, retries, cancelación, métricas, healthchecks, mTLS, almacenamiento compartido y persistencia durable de estado y eventos. La propuesta se enfoca principalmente en la capa worker y en su integración con el plano de coordinación. El resultado es un prototipo ejecutable, contenedorizado y validado por pruebas automatizadas y una demo segura extremo a extremo.
\end{abstract}
...
Quedaron fuera de alcance: frontend, autenticación de usuario final, base de datos de negocio completa y un backend corporativo ajeno a la capa distribuida. Para validación local y despliegue se implementó un clúster de coordinadores activo/pasivo con HAProxy, Redis y MinIO, suficiente para demostrar failover, despacho global y storage compartido obligatorio.

\section{Arquitectura general del prototipo}
La Fig.~\ref{fig:architecture} resume la arquitectura ejecutable del prototipo: un clúster coordinador activo/pasivo publicado por un endpoint gRPC único, tres workers con la misma imagen de despliegue y un backend de almacenamiento compartido.
...
\node[service] (coord) at (0,4.6) {Clúster coordinador\\HAProxy + Redis};
...
\end{tikzpicture}
\caption{Arquitectura del prototipo. Azul: RPC de control desde el coordinador activo hacia cada worker. Verde: callbacks del worker hacia el endpoint gRPC publicado por el clúster coordinador. Gris: acceso a storage, métricas y healthchecks.}
...
\item mTLS obligatorio en el despliegue contenedorizado y opcional en ejecución local;
\item despliegue local con \texttt{docker compose};
\item tres instancias worker, dos coordinadores activo/pasivo, Redis, MinIO y HAProxy.
\end{itemize}

El estado de readiness del worker considera cola, memoria disponible y conectividad hacia el coordinador. En el coordinador, \texttt{/readyz} representa liderazgo activo y permite que HAProxy publique solo la instancia líder. Las métricas más relevantes incluyen longitud de cola, tiempos de espera, latencia de procesamiento, retry count, utilization de recursos, tareas activas, cancelaciones y conectividad gRPC.
...
La ejecución final de la suite arrojó veintidós pruebas aprobadas. Adicionalmente se validó una demo segura extremo a extremo sobre el endpoint publicado por HAProxy y se simuló la caída del coordinador líder, observando toma de liderazgo del follower sin cambiar el endpoint público. Esto confirma que el worker y el clúster coordinador son ejecutables, capaces de aceptar tareas, encolarlas, despacharlas, procesar imágenes, reportar progreso y recuperarse ante fallos del plano de control.

\section{Cierre del prototipo}
\noindent\textbf{Capacidades consolidadas.}
...
\item coordinación global con cola central durable, ownership por solicitud, deduplicación y despacho entre workers;
\item soporte para almacenamiento compartido obligatorio en el despliegue contenedorizado, búsqueda de resultados, lotes y extensiones concretas de OCR e inferencia.
\end{itemize}

En el resultado final, el balanceo global queda centralizado en el coordinador y el procesamiento intensivo se mantiene aislado en los workers. Esta separación conserva responsabilidades claras: el coordinador decide, despacha y agrega estado del clúster, mientras cada worker prioriza, admite, ejecuta, persiste y reporta sus tareas.

\noindent\textbf{Proyección de evolución.}
Sobre esta base ya funcional pueden añadirse, sin rehacer la arquitectura del worker, modos activo-activo con consenso distribuido, políticas de asignación más sofisticadas y modelos de inferencia de mayor tamaño.

\section{Conclusiones}
Se implementó exitosamente un subsistema de workers distribuidos para procesamiento de imágenes con Python, \texttt{asyncio} y gRPC. El resultado no es un diagrama teórico sino un prototipo ejecutable con tres nodos worker, clúster coordinador con failover real, storage compartido, control de admisión, cancelación, retries, observabilidad, OCR concreto con Tesseract, inferencia concreta y despliegue seguro en contenedores. La arquitectura lograda separa de forma clara coordinación, scheduling, ejecución y reporte, lo que la vuelve apta como base para una evolución posterior hacia un sistema distribuido productivo.
