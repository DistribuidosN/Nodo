...
\item almacenamiento local por nodo;
...
Storage abstracto & \texttt{storage.py} & Local, \texttt{file://} y \texttt{http(s)} de solo lectura \\
...
Despliegue & \texttt{docker-compose.yml} & Tres workers con almacenamiento local \\
\bottomrule
\end{longtable}
...
\textbf{Qué hace}
\begin{itemize}
\item materializa entradas, salidas y estado en disco local;
\item levanta \texttt{worker1}, \texttt{worker2} y \texttt{worker3};
\item usa almacenamiento local por nodo;
...
\item puertos públicos para gRPC, salud y métricas.
\end{itemize}

\textbf{Cómo se conecta}
\begin{itemize}
\item el almacenamiento del nodo es local y persistente en su propio volumen;
\item los workers exponen gRPC por mTLS;
\item cada worker expone sus propias métricas y healthchecks.
\end{itemize}
...
\textbf{Qué hace}
\begin{itemize}
\item carga puertos, límites, almacenamiento local, TLS, tracing, OCR e inferencia;
...
\item \texttt{WORKER\_INPUT\_DIR}, \texttt{WORKER\_OUTPUT\_DIR} y \texttt{WORKER\_STATE\_DIR}.
\end{itemize}
...
\item persistencia de estado local;
...
\item soporta local, \texttt{file://} y \texttt{http(s)} para lectura externa.
\end{itemize}
...
\textbf{Qué hace}
\begin{itemize}
\item expone métricas HTTP por worker;
...
\item prepara certificados y llaves para TLS/mTLS de desarrollo.
\end{itemize}
...
\item el sistema usa gRPC, mTLS y almacenamiento local por nodo;
